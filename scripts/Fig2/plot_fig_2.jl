...
save(plotsdir("int_kitaev_phase_diagram_wavefunctions_$(N).png"), fig_aps, px_per_unit=3)


## with disorder

disorder = wload(datadir("int_kitaev_disorder_$N.jld2"))
fig_aps = with_theme(theme_aps(markers=[:circle, :diamond, :utriangle], linestyles=[nothing, :dash])) do
    x = phase_diagram["Us"]
    y = phase_diagram["δμs"]
    bounds = phase_diagram["bounds"]
    ...
    fig = Figure(figure_padding=(13, 8, 1, 5), size=220 .* (1.2, 1.3))
    ga = fig[2, 1] = GridLayout()
    ax1 = Axis(ga[1, 1]; xlabel=L"U/t", ylabel=L"%$yparam/t", title=L"\sqrt{1-M_\text{half}}")
    ax2 = Axis(ga[1, 2]; xlabel=L"U/t", title=L"||(i\gamma \tilde{\gamma})_\text{half}||_2")
    linkaxes!(ax1, ax2)
    ...
    ax = Axis(gb[1, 1]; yscale, xlabel=L"j", limits, xticks=1:N)
    text!(fig.scene, 0.002, 0.92; text=LaTeXString("(a)"), space=:relative)
    text!(fig.scene, 0.002, 0.6; text=LaTeXString("(b)"), space=:relative)
    text!(fig.scene, 0.002, 0.25; text=LaTeXString("(c)"), space=:relative)
    ...
    rowgap!(fig.layout, 1, Relative(0.02))
    colsize!(gb, 2, Relative(0.24))

    gc = fig[3, 1] = GridLayout()
    σs = disorder["σs"]
    normalization = disorder["params"].t
    xs = σs / normalization
    ys = disorder["rms"] / normalization
    axc = Axis(gc[1, 1], xlabel=L"\sigma/t", ylabel=L"E /t")
    ylims!(axc, 0, 1.1 * maximum(ys))
    xlims!(axc, 0, maximum(xs))
    lines!(axc, xs, ys; label=L"\sqrt{\overline{\delta E^2}}", color=Cycled(1), linewidth=3)
    lines!(axc, xs, (σs * Qe) / normalization, label="1:st order bound"; linestyle=:dash, color=Cycled(4), linewidth=3)
    axislegend(axc, position=(:left, :top), labelsize=9)

    fig
end
save(plotsdir("int_kitaev_phase_diagram_wavefunctions_disorder_$(N).pdf"), fig_aps)
save(plotsdir("int_kitaev_phase_diagram_wavefunctions_disorder_$(N).png"), fig_aps, px_per_unit=3)
